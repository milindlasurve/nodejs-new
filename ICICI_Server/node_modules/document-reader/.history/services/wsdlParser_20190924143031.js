...
async function readWsdl(wsdlData){
	...
	for(var key in wsdlData){
		...
		if(key){
			allObjects.push({operations:operationsArray});
			return allObjects;
		}
	}
...
/* Iterates over the wsdl tree stucture recurively. */
			...
			if(!val){
				if(treeLevel==1){
					...
					/* Variable to store Request or Response Type. It changes after calling flatten method */
						...
						operationsArray.push({operationId:operationParam,fields:tree,exchanges:reqRes});
						break;			
					}
				 }				
...
/* Will stop the iteration after it reaches the final tree in a structure */
...
/* Flattens a hierarchial tree structure */
async function flatten(data) {
    var result = {};
    function recurse (cur, prop) {
        if (Object(cur) !== cur) {
            result[prop+".type"] = cur;
...
/* UnFlattens JSON Tree structure */
...
/* Captures the data type of parameters. */
