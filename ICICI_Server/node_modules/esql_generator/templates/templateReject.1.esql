...
CREATE COMPUTE MODULE Ecollection_LEND_File_To_JSON_Response
	...
	BEGIN
		...
		Reject : WHILE (i <= statusLength) DO
			...
			IF( UPPER (InputRoot.JSON.Data.-- Change --) = Environment.Variables.Status[i].REFUND_STATUS) THEN
			...
			ELSEIF( UPPER (InputRoot.JSON.Data.-- Change --) = Environment.Variables.Status[i].FT_STATUS) THEN
