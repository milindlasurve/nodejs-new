...
//createEsqlISurePayResponse(template, parameters, fileName)
 ...
 */
async function createEsqlISurePayResponse(template, parameters, fileName) {
    ...
    try {
       ...
       // console.log("FIELD LENGTH ::: ", (parameters.fieldDefinitions).length);
...
async function createParameterDefinitions(parameters, sourceType, targetType) {
    ...
    try {
        ...
        var esqlLHS = await setLHSValue(esqlLHSValues, parameters, values, sourceType)

        // // Capture Values for RHS

        var esqlRHS = await setRHSValue(esqlRHSValues, parameters, targetType);
        ...
        //     console.log("esqlRHSVALUES :::: ",esqlRHSValues);
        // }
...
// 		}
// 		else if(type === "XML:REF"){
// 			preset = "refValidationRequest.REQ."
// 		}
// 		else { }
// 		result = preset + param + ";"
...
async function setLHSValue(esqlLHSValues, parameters, param, type) {
    ...
    try {
        for (var i in parameters) {
            ...
            //console.log("Value : ", parameters[i].source);
            param = (parameters[i].source).split(".");

            for (var j = 0; j < param.length; j++) {
                if (param[j] === "items") {
                    param[j] = "item";
                } else if (param[j] === "properties") {
            ...
            if (type === "JSON") {
                ...
                preset = "SET OutputRoot.XMLNSC.RESP.";
                result = preset + value
                esqlLHSValues.push(result);
                result = preset + value + ".(XMLNSC.Attribute)labelname";
                esqlLHSValues.push(result);
                result = preset + value + ".(XMLNSC.Attribute)dispflg";
                esqlLHSValues.push(result);
            ...
            //return preset+value;
            if (type !== "XML:JSON") {
                esqlLHSValues.push(preset + value);
            }
        }
        //console.log("ESQL LHS VALUES ::: ", esqlLHSValues);
        return esqlLHSValues;
...
async function setRHSValue(esqlRHSValues, parameters, type) {
    ...
    try {
        ...
        var operation = "";
        for (var i in parameters) {
            param = parameters[i].target;
            operation = parameters[i].operation
            // console.log("RHS Value : ", parameters[i].target);
            // console.log(type);
            var preset = "";
            if (type === "DFDL") {
            ...
            // For XML where 3 lines of code are required
            else if (type === "XML") {
                preset = "InputRoot.JSON.Data.";
                result = preset + param + ";"
                esqlRHSValues.push(result);
                result = "'" + param + "';"
                esqlRHSValues.push(result);
                result = "'Y';\n";
                esqlRHSValues.push(result);
       ...
       // console.log("RHS VALUES ::: ", esqlRHSValues);
        return esqlRHSValues;
    }
    catch (e) { throw e; }
}
...
module.exports.createEsqlISurePayResponse = createEsqlISurePayResponse
